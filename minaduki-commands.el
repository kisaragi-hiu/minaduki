...
(defun minaduki-add-tag ()
  "Add a tag."
  (interactive)
  (let* ((all-tags (minaduki-edb::fetch-all-tags))
  ...
  (let ((index (cond
                ...
                ((stringp minaduki:index-file)
                 (f-expand minaduki:index-file
                           (minaduki-vault:main)))
                (t
                 (car (minaduki-edb::fetch-file :title "Index"))))))
...
one."
  ...
  (when (stringp entry)
    (setq entry
          (minaduki-node
           :path (car (minaduki-edb::fetch-file :title entry))
           :title entry)))
...
current window."
  (-when-let (id (minaduki-edb::fetch-id id))
...
process."
  (let* ((file (minaduki-edb::fetch-file :key citekey))
         (_title (minaduki-edb::fetch-title file)))
    (cond
     (file (minaduki::find-file file))
     (t (let ((props (or (-some-> (minaduki-edb::fetch-lit-entry citekey)
...
(defun minaduki/visit-source (citekey)
  ...
  (let ((entry (minaduki-edb::fetch-lit-entry citekey))
        sources)
...
(defun minaduki:insert-note-to-citekey (citekey)
  ...
  (-if-let* ((path (minaduki-edb::fetch-file :key citekey))
             (title (minaduki-edb::fetch-title path)))
