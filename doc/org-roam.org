...
Org-roam calls ~org-roam--extract-titles~ to extract titles.
...
- User Option: minaduki-buffer

  The name of the org-roam buffer. Defaults to ~*org-roam*~.

- User Option: minaduki-buffer/position

  The position of the Org-roam buffer side window. Valid values are ~'left~,
  ~'right~, ~'top~, ~'bottom~.

- User Option: minaduki-buffer/width

  Width of ~minaduki-buffer~. Has an effect only if ~minaduki-buffer/position~ is
  ~'left~ or ~'right~.

- User Option: minaduki-buffer/height

  Height of ~minaduki-buffer~. Has an effect only if ~minaduki-buffer/position~ is
  ~'top~ or ~'bottom~.

- User Option: minaduki-buffer/window-parameters
  ...
  ~(setq minaduki-buffer/window-parameters '((no-delete-other-windows . t)))~
...
#+BEGIN_SRC emacs-lisp
(require 'kisaragi-notes-org-protocol)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  used as title."
      (el-patch-swap (if deft-use-filename-as-title
                     ...
                     (kisaragi-notes-db//fetch-title file))))
#+END_SRC
